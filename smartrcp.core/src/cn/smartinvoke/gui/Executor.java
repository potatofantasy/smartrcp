...
public class Executor {
	 ...
	 try{
		if (command != null) {
			...
			if (reqPack == null) {//报文错误
				...
				ret=ProtocolBuilder.getProtocolStr(e,this.appId);
			}
			...
			//RuntimeException e=new RuntimeException(Messages.ERROR_PACKAGE);
			ret=ProtocolBuilder.getProtocolStr(new InvokeException(Messages.ERROR_PACKAGE),this.appId);
		}
	 }catch(Throwable e){
		 ret=ProtocolBuilder.getProtocolStr(new InvokeException(e.getMessage()),this.appId);
	 }
	...
	public String create(String pack)throws Exception {
		String ret =null;
		Object[] ivkInfo=(Object[])ProtocolBuilder.getObject(pack,this.appId);
		...
		ret=ProtocolBuilder.getProtocolStr(objId,this.appId);
		return ret;
	}
    /**
     * 请求报文是一数组
	...
	public String call(String pack)throws Exception {
		...
		return ProtocolBuilder.getProtocolStr(retObj,this.appId);
	}
